...
for model, params in zip(models, param_model):
    ...
    for n_features in feature_range:
        ...
        pipe = Pipeline([
            ('Msurfs', MultiSURFstar( n_features_to_select=n_features)),
    ...
    final_pipe = Pipeline([
        ('Msurfs', MultiSURFstar( n_features_to_select=best_params['n_features_to_select'])),
        ('smote', SVMSMOTE(sampling_strategy='minority', random_state=42)),
        ('model', final_model)])
...
for model, params in zip(models, param_model):
    ...
    for n_features in feature_range:
        ...
        logistic = LogisticRegression(penalty='l1', solver='liblinear', random_state = 693)
        param_grid = {'C': np.arange(0.1,1.,0.1)}
        # Configuration de la recherche par grille avec une validation croisée à 3 plis
        ...
        lr = LogisticRegression(**grid_search.best_params_, random_state = 693)
        pipe = Pipeline([
    ...
    final_model = type(model)(**model_params)
    logistic = LogisticRegression(penalty='l1', solver='liblinear', random_state = 693)
    param_grid = {'C': np.logspace(-6, 1, 100)}
        # Configuration de la recherche par grille avec une validation croisée à 3 plis
    grid_search = GridSearchCV(logistic, param_grid, cv=3, scoring='accuracy')
        # Exécution de la recherche par grille sur les données
    grid_search.fit(X_train, np.ravel(Y_train))
    lr = LogisticRegression(**grid_search.best_params_, random_state = 693)
    final_pipe = Pipeline([
        ('sfs', SequentialFeatureSelector(lr, n_features_to_select=best_params['n_features_to_select'])),
        ('smote', SVMSMOTE(sampling_strategy='minority', random_state=42)),
        ('model', final_model)])
