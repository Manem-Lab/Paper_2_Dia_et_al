...
for model, params in zip(models, param_model):
    ...
    for n_features in feature_range:
        ...
        pipe = Pipeline([
            ('surfS', SURFstar( n_features_to_select=n_features)),
    ...
    final_pipe = Pipeline([
        ('surfS', SURFstar( n_features_to_select=best_params['n_features_to_select'])),
        ('smote', SVMSMOTE(sampling_strategy='minority', random_state=42)),
        ('model', final_model)])
...
models = [SVC(random_state = 693), LogisticRegression(random_state = 693),
         LinearDiscriminantAnalysis(), KNeighborsClassifier(), GaussianNB()]
...
for model, params in zip(models, param_model):
    ...
    for n_features in feature_range:
        # Créer un pipeline avec ReliefF, SVMSMOTE et le modèle
        pipe = Pipeline([
            ('Msurf', MultiSURF( n_features_to_select=n_features)),
            ('smote', SVMSMOTE(sampling_strategy='minority', random_state=42)),
            ('model', model)
        ])
    ...
    final_model = type(model)(**model_params)
    final_pipe = Pipeline([
        ('Msurf', MultiSURF( n_features_to_select=best_params['n_features_to_select'])),
        ('smote', SVMSMOTE(sampling_strategy='minority', random_state=42)),
        ('model', final_model)])
