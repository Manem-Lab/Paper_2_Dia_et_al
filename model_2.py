...
for model, params in zip(models, param_model):
    ...
    for n_features in feature_range:
        ...
        pipe = Pipeline([
            ('relief', ReliefF(n_neighbors=5, n_features_to_select=n_features)),
    ...
    final_pipe = Pipeline([
        ('relief', ReliefF(n_neighbors=5, n_features_to_select=best_params['n_features_to_select'])),
        ('smote', SVMSMOTE(sampling_strategy='minority', random_state=42)),
        ('model', final_model)])
...
validation_scores = []
n_splits = 5  # Nombre de plis pour la validation croisée
feature_range = range(3, 52)  # Plage pour le nombre de caractéristiques à sélectionner
n_iter = 100  # Nombre d'itérations pour RandomizedSearchCV
for model, params in zip(models, param_model):
    best_score = 0
    best_params = {'n_features_to_select': None, 'params': None}
    ...
    final_model = type(model)(**model_params)
    final_pipe = Pipeline([
        ('anova', SelectKBest(f_classif, k=best_params['n_features_to_select'])),
        ('smote', SVMSMOTE(sampling_strategy='minority', random_state=42)),
        ('model', final_model)])
